...
const SalonServicesPage = () => {
  ...
  const handleFormSubmit = async (serviceData) => {
    ...
    // AUTO-INJECT THE CURRENCY FROM PROFILE
    const payload = {
      ...serviceData,
  ...
  if (!salonData) {
    return (
      <div className="p-10 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-gray-500 mb-8">Please complete your salon profile and select your local currency first.</p>
  ...
  return (
    <div className="p-6 md:p-10 space-y-8">
          ...
          <tbody className="divide-y divide-gray-50">
            {salonData.services?.map((service) => (
              <tr key={service._id} className="hover:bg-gray-50/50 transition-all">
                ...
                <td className="p-6 text-gray-500 text-sm">{service.duration}</td>
      ...
      {isModalOpen && (
        <ServiceModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleFormSubmit} initialData={editingService} />
      )}
    </div>
  );
};
