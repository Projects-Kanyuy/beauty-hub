...
const buildSwychrCallbackUrl = (req, paymentId, subscriptionId, planId) => {
  const baseUrl = getRequestBaseUrl(req);
  if (!baseUrl) return null;
  const url = new URL(`${baseUrl}/api/payments/swychr/webhook`);
  url.searchParams.set("transaction_id", paymentId);
  if (subscriptionId) {
...
const subscribe = asyncHandler(async (req, res) => {
  ...
  if (!planId) {
    console.warn("[subscribe] Missing planId", {
      userId: user?._id,
      body: req.body,
    });
  ...
  try {
    ...
    if (!plan) {
      console.warn("[subscribe] Invalid planId", {
        planId,
        userId: user?._id,
      });
    ...
    if (!["XAF", "USD"].includes(plan?.currency)) {
      console.warn("[subscribe] Unsupported plan currency", {
        planId,
      ...
      return res.status(400).json({
        message:
          "Only XAF and USD payments are supported right now. Please update the plan currency.",
      });
    }
    ...
    const createdSubscriptions = await Subscription.create(
      [subscriptionPayload],
      { session },
    );
    ...
    const createdPayments = await Payment.create([paymentPayload], { session });
    const createdPayment = createdPayments[0];

    // Step 3: initiate the payment
    if (!process.env.SWYCHR_EMAIL || !process.env.SWYCHR_PASSWORD) {
      console.error("[subscribe] Missing Swychr credentials", {
        hasEmail: Boolean(process.env.SWYCHR_EMAIL),
        hasPassword: Boolean(process.env.SWYCHR_PASSWORD),
      });
      await session.abortTransaction();
      session.endSession();
    ...
    const callbackUrl =
      buildSwychrCallbackUrl(
        req,
        createdPayment._id.toString(),
        createdSubscription._id.toString(),
        planId,
      ) || process.env.SWYCHR_CALLBACK_URL;
    if (callbackUrl) {
      payload.callback_url = callbackUrl;
    }

    const swychrResponse = await createPaymentLink(
      token,
      payload,
      createdPayment._id.toString(),
    );
    ...
    console.error("[subscribe] Payment flow failed", {
